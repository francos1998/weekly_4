...
ggmap(world)+
  ...
  labs(title = "Starbucks around the world")
...
ggmap(US)+
  ...
  theme_map()+
  theme(legend.background = element_blank())+
  labs(title = "Ridership of bike Stations in Washington DC",
       color = "Rides per Station")
...
ggmap(WA)+
  ...
  theme_map()+
  theme(legend.background = element_blank())+
  labs(title = "Casual users proportion for stations in Washington DC",
  color = "Casual users' 
      proportion")
```
...
covid19_latest %>% 
  ...
  labs(title = "States' most recent cumulative count of COVID cases")

```
  
>The problem with this map is that it is not accounting for differences in population. Therefore, states with bigger populations seem to have higher case counts. 
...
mpls_suspicious <- 
  MplsStops %>% 
    ...
    summarize(prop_suspicious = mean(problem == "suspicious"),
              freq_stops = n()) %>% 
    arrange(desc(freq_stops))
mpls_suspicious
```
...
```{r}
pal <- colorFactor(c("green","yellow"),
...
>My question is: What is the spatial income distribution across neighborhoods?
...
mpls_all %>% 
  leaflet() %>% 
    addProviderTiles(providers$CartoDB.Voyager) %>% 
    addPolygons(label = ~paste(BDNAME,",",hhIncome,"$"),
                color = ~pal(hhIncome),
                opacity = 1) 
```

>My map shows the income per household spatial distribution, we can observe that wealthier areas are further from the center of the city, being Northloop the wealthiest neighborhood that is closest to the middle of the city. Also, there seem to be wealthier neighborhoods at the Southern part of Minneapolis than at the north.  
  
## GitHub link

  19. Below, provide a link to your GitHub page with this set of Weekly Exercises. Specifically, if the name of the file is 04_exercises.Rmd, provide a link to the 04_exercises.md file, which is the one that will be most readable on GitHub.

[This is the link](https://github.com/francos1998/weekly_4/blob/main/04_exercises.md)
